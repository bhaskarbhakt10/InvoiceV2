...
if (!empty($invoice_info_json)) {
    ...
    foreach ($invoice_info_arr as $info_arrkey => $info_arrvalue) {
        if ($info_arrvalue['uniqueID'] === $perfoma_id) {
            ...
            $total = preg_replace('/,/', '', $info_arrvalue['total']);
        }
    }
}
// echo $proforma_text;
if (!empty($client_details_json)) {
    ...
    } else if (!empty($client_details_arr['address-country-city'])) {
    ...
    if (strcasecmp($textbox_useage, 'yes') === 0) {
        ...
        $client_address .= $client_details_arr['address-line-one'] . ', ';
        $client_address .= !empty($client_details_arr['address-line-two']) ? '<br>' . $client_details_arr['address-line-two'] . ',<br>' : '';
        $client_address .= $city;
        $client_address .= !empty($client_details_arr['postal-code']) ? $client_details_arr['postal-code'] . ', ' : '';
        $client_address .= $client_details_arr['address-country-state'] . ', ' . $client_details_arr['address-country'];
    }
}
...
if (!empty($description) && !empty($price)) {
    ...
    $exploded_price__ = explode('<=>', $price);
    $exploded_price = preg_replace('/,/', '', $exploded_price__);
    // print_r($exploded_price);
    for ($i = 0; $i < count($exploded_description); $i++) {
        if (!empty($exploded_description[$i])) {
            $sr_no = $i + 1;
            $description_row .= '<tr><td class=" border-table text-indent-left text-center">' . $sr_no . '</td><td class="border-table text-indent-left">' . $exploded_description[$i] . '</td><td class="border-table text-indent-left text-center">998314</td><td class="border-table text-indent-left text-right">' . number_format($exploded_price[$i], 2, '.', ',') . '</td></tr>';
        }
    }
}
...
$subtotal_data = '';
if (!empty($subtotal)) {
    $subtotal_data .= '<td class="border-table text-right text-indent-left ">' . number_format($subtotal, 2, '.', ',') . '</td>';
}
...
if (!empty($IGST) && !empty($IGST_value)) {
    $igst_row = '<tr><td></td><td></td><td class=" border-table text-right text-indent-left"><b>IGST (' . $IGST . ')</b></td><td class=" border-table text-right text-indent-left">' . number_format($IGST_value, 2, '.', ',') . '</td></tr>';
}
$sgst_row = '';
if (!empty($SGST) && (!empty($SGST_value))  && !empty($CGST) && !empty($CGST_value)) {
    $sgst_row .= '<tr><td colspan="2"></td><td class="border-table text-right text-indent-left"><b>CGST (' . $CGST . ')</b></td><td class="border-table text-right text-indent-left">' . number_format($CGST_value, 2, '.', ',') . '</td></tr>';
    $sgst_row .= '<tr><td colspan="2"></td><td class="border-table text-right text-indent-left"><b>SGST (' . $SGST . ')</b></td><td class="border-table text-right text-indent-left">' . number_format($SGST_value, 2, '.', ',') . '</td></tr>';
}
$total_data = '';
if (!empty($total)) {
    $total_data .= '<td class="color-orange border-table text-right text-indent-left">' . number_format($total, 2, '.', ',') . '</td>';
}
...
if (!empty($service)) {
    switch ($service) {
        case 'SMO':
            $tnc = '<tr>
            <td class="font-10"><b>Payment Terms:</b>
                <ol class="">
        ...
        case 'AMC':
            $tnc = '<tr>
            <td class="font-10"><b class="">Payment Terms:</b>
                <ol class="">
...
class MYPDF extends TCPDF
{
    ...
    public function Header()
    {
        ...
        $html .= '<table class=""><tr><td ><img src="' . $header_image . '" class="header-image"></td></tr></table>';
    ...
    public function Footer()
    {
        ...
        $html = '';
        $html .= '<style>.text-right{text-align:right;}ol{padding:0; line-height:0; list-style:}li{line-height:12px;}.blue{background-color:blue;}.line-footer{color:#ff6d3b;height:2px}.red{background-color:red;}.w-10{width:10%;}.W-80{width:80%;}.font-10{font-size:10px;}.footer-image{ width:950px; }.for-nuit-sol{font-size:13px; line-height:25px;}</style>';
        $html .= '<table class=""><tr><td class="w-10"></td><td class="w-80"><table><tr><td class="text-right for-nuit-sol"><b>For NUIT Solutions</b></td></tr><tr><td><p class="text-right"><i>*This is an Electronic ' . $this->invoice_or_profoma . ' and hence needs no signature.</p></i><hr class="line-footer"></td></tr><tr><td class="font-10">NUIT Solutions proclaims 100% Non-Disclosure Agreement Guarantee to prevent unauthorized access of client-owned “Confidential Information”. We assert that it will never be used for any of our other clients</td></tr>' . $this->tnc . '</table></td><td class="w-10"></td></tr></table>';
        $html .= '<table class="w-100"><tr><td><img src="' . $footer_image . '" class="footer-image"></td></tr></table>';
        $this->writeHTML($html, false, true, false, false, 'L');
    }
}
...
if (!empty($service)) {
    switch ($service) {
        case 'SMO':
            $pdf->SetFooterMargin(108);

            break;
        case 'SEO':
            $pdf->SetFooterMargin(105);
...
$main_body .= '<table class="w-100 table"><tbody><tr><td class="FONT14"><b class="">' . $proforma_text . ' </b>' . $perfoma_number . '</td></tr><tr><td class="FONT14"><b>Invoice Date: </b>' . $dated_invoice . '<br></td></tr><tr><td class="FONT14"><b>Invoiced To</b><br>' . $client_name . '<br>' . $client_address . '</td></tr></tbody></table>';
$main_body .= '</td>';
$main_body .= '<td class="w-30">';
$main_body .= '<table class="w-100 table"><tbody><tr><td class="text-right FONT14"><b>NUIT SOLUTIONS</b><br>Sanghamitra, Anmol Nagar<br>Naigaon West<br>District Palghar 401207<br>GSTIN 27APXPJ2589P2ZM</td></tr></tbody></table>';

$main_body .= '</td>';
$main_body .= '</tr></table><div></div>';

$main_body .= '<table><tr><th class="text-center FONT14 "><b>' . $proforma_heading . '</b></th></tr></table><div></div>';
$main_body .= '<table cellpadding="3" class="w-100 main-table" id="main-table"><tr><th class="text-center w-10 border-table"><b>SR NO.</b></th><th class="text-center w-50 border-table"><b>PARTICULARS</b></th><th class="border-table text-center w-20"><b>SAC CODE</b></th><th class="border-table text-center w-20"><b>AMOUNT<br>(in INR)</b></th></tr>';
$main_body .= $description_row;
$main_body .= '<tr><td colspan="2" class="hide-td"></td><td class="border-table text-right text-indent-left"><b>Sub Total</b></td>' . $subtotal_data . '</tr>';
$main_body .= $igst_row;
$main_body .= $sgst_row;
$main_body .= '<tr><td colspan="2" class="hide-td"></td><td class="color-orange border-table text-right text-indent-left"><b>Total</b></td>' . $total_data . '</tr>';
